...
| ADR-0009 | Two-model architecture, fast and code | Candidate predecessor baseline material plus v3.2.4.4 amendment | accepted in candidate source; amended | active constraint | recovered amendment, not runtime implementation | RamaLama is named as the local-SLM runtime; cloud-only, local-only, and hybrid inference are first-class tenant topology choices. Replay never re-inferences. |
