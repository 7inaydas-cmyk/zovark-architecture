...
def test_ingested_at_is_deterministic_across_repeated_runs():
    raw = load_sample(SAMPLE_PATH)

    first = normalize_evidence(raw)
    second = normalize_evidence(raw)

    assert [entry["ingested_at"] for entry in first] == [
        entry["ingested_at"] for entry in second
    ]
    assert first == second


def test_source_timestamp_is_preferred_over_top_level_alert_timestamp():
    raw = {
        "alert_id": "alert-source-time",
        "timestamp": "2026-05-01T10:00:00Z",
        "process_events": [
            {
                "event_id": "pe-source-time",
                "event_type": "process_event",
                "observed_at": "2026-05-01T10:00:01Z",
            }
        ],
    }
    ...
    assert entries[0]["ingested_at"] == "2026-05-01T10:00:00Z"
    assert entries[1]["ingested_at"] == "2026-05-01T10:00:01Z"


def test_top_level_alert_timestamp_is_used_as_event_fallback():
    raw = {
        "alert_id": "alert-fallback-time",
        "timestamp": "2026-05-01T10:00:00Z",
        "process_events": [
            {"event_id": "pe-fallback-time", "event_type": "process_event"}
        ],
    }

    entries = normalize_evidence(raw)

    assert entries[1]["ingested_at"] == "2026-05-01T10:00:00Z"
    assert "timestamp" not in entries[1]["raw_content"]
...
def test_missing_process_and_network_arrays_do_not_fail_when_alert_exists():
    entries = normalize_evidence(
        {"alert_id": "alert-only", "timestamp": "2026-05-01T10:00:00Z"}
    )
...
def test_missing_deterministic_timestamp_raises_validation_error():
    with pytest.raises(ZovarkValidationError):
        normalize_evidence({"alert_id": "alert-no-time"})
